    ...
    <li className="task-item" style={{ borderLeft: `8px solid ${task.color}` }}>
      <div className="task-main">
        <div className="title-row">
          <h3>{task.title}</h3>
          <span className="badge" style={{ background: statusBadgeColor(effectiveStatus) }}>
            {effectiveStatus}
          </span>
        </div>
      ...
      <div className="actions">
        ...
        {/* Mark Completed sau Reopen */}
        {effectiveStatus !== "Completed" ? (
          <button onClick={() => onUpdate(task.id, { status: "Completed" })}>Mark Completed</button>
        ) : (
          <button onClick={() => onUpdate(task.id, { status: "Upcoming" })}>Reopen</button>
        )}

        {/* Cancel sau Reopen */}
        {effectiveStatus !== "Canceled" ? (
          <button className="ghost" onClick={() => onUpdate(task.id, { status: "Canceled" })}>Cancel</button>
        ) : (
          <button className="ghost" onClick={() => onUpdate(task.id, { status: "Upcoming" })}>Reopen</button>
        )}
